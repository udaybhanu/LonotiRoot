...
import com.android.lonoti.bom.Response;
import com.android.lonoti.bom.payload.LonotiEvent;
import com.android.lonoti.bom.payload.FacebookLogin;
...
import com.android.lonoti.bom.payload.Register;
...
public class RequestCreator {
	private static Request getRequest(Object obj) {
		...
		if(obj instanceof JSONObject){
			...
			if(Request.REGISTER.equals(type)){
				...
				payload = new LonotiEvent();
				((LonotiEvent)payload).setName(payloadObj.get("name").toString());
				((LonotiEvent)payload).setDescription(payloadObj.get("description").toString());
				...
				if(locationObj != null){
					...
					((LonotiEvent)payload).setLocation(location);
				}
				...
				if(friendsObj != null){
					...
					while(it.hasNext()){
						...
						((LonotiEvent)payload).getFriends().add(friend);
					}
				}
	...
	public static String convertResponsetoJSON(Request request){
		...
		if(Request.LOGIN.equals(type)){
			...
			payloadObject.put("username", login.getEmail());
			payloadObject.put("password", login.getPassword());
			requestObj.put("type", "login");
			...
			LonotiEvent event = (LonotiEvent) request.getPayload();
